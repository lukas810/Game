...
public class Slots {
    ...
    public void render(Graphics2D g) {
        ...
        if(button != null) {
            ...
            g.drawImage(slot[0], (int) pos[0].getX(), (int) pos[0].getY(), width, height, null);
        }
        for(int i = 1; i < slot.length; i++) {
            if(slot[i] == null) continue;
            g.drawImage(slot[i],
                        (int) pos[i].getX() - ((slot[i].getWidth() + size) / slot[i].getWidth()) + 1,
                        (int) pos[i].getY() - ((slot[i].getHeight() + size) / slot[i].getHeight()) + 1,
