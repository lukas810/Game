...
public class Enemy extends Entity {
	...
	public void update(Player player, double time) {
		if (camera.getBounds().collides(this.bounds)) {
			...
			sense = new AABB(new Vector2f(pos.getX() + size / 2 - r_sense / 2, pos.getY() + size / 2 - r_sense / 2),
					r_sense);
			attackrange = new AABB(
					new Vector2f(pos.getX() + bounds.getxOffset() + bounds.getWidth() / 2 - r_attackrange / 2,
